...
class RetrievalHandler(TaskHandler):
    ...
    def eval_batch(self, model, image, language_tokens, padding_mask, image_id, imgid):
        ...
        self.imgids.append(imgid.clone())
    ...
    def after_eval(self, **kwargs):
        dataset_name = "new_reftext"
        split = ["train", "val", "test"]
        # jsonl_file_path = f"data/coco_retrieval.{split}.jsonl"
        jsonl_file_path_list = [
            f"/home/initial/switching_reverie_retrieval/data/{dataset_name}/beit3_coco/coco_retrieval.{split}.jsonl"
            for split in split
        ]
        beit3_dir = f"/home/initial/switching_reverie_retrieval/data/{dataset_name}/beit3"
        os.makedirs(beit3_dir, exist_ok=True)

        jsonl_data = []

        for jsonl_file_path in jsonl_file_path_list:
            jsonl_data.extend([json.loads(line) for line in open(jsonl_file_path, "r")])
        # with open(jsonl_file_path, "r") as f:
        #     jsonl_data = [json.loads(line) for line in f]
        image_feats = {}
        text_fiats = {}
        unique_imgids = {}
        for i_feats, t_fiats, ids, u_imgids in zip(self.image_feats, self.text_feats, self.image_ids, self.imgids):
            for i, _idx in enumerate(ids):
                idx = _idx.item()
                if idx not in image_feats:
                    image_feats[idx] = i_feats[i]
                    text_fiats[idx] = t_fiats[i]
                    unique_imgids[idx] = u_imgids[i]

                    for data in tqdm(jsonl_data):
                        if data["image_id"] == idx:
                            image_path = data["image_path"]
                            file_path = image_path.split("/")[-1].split(".")[0]
                            np_image_feats = image_feats[idx].cpu().detach().numpy()
                            np_text_feats = text_fiats[idx].cpu().detach().numpy()
                            unique_imgid = unique_imgids[idx]

                            image_feats_path = os.path.join(beit3_dir, f"{unique_imgid}_image.npy")
                            text_feats_path = os.path.join(beit3_dir, f"{unique_imgid}_text.npy")
                            np.save(
                                image_feats_path,
                                np_image_feats,
                            )
        ...
        ranks = []
        for idx in range(scores.size(1)):
            target_id = tiids[idx].item()
            sorted_scores, sorted_indices = scores[:, idx].sort(descending=True)
            rank = (iids[sorted_indices] == target_id).nonzero(as_tuple=True)[0].item() + 1  # Rank is 1-based
            ranks.append(1.0 / rank)
        mrr_tr = torch.tensor(ranks).mean().item()

        # topk20 = scores.topk(20, dim=1)
        # topk10 = scores.topk(10, dim=1)
        # topk5 = scores.topk(5, dim=1)
        # topk1 = scores.topk(1, dim=1)

        # topk20_iids = tiids[topk20.indices]
        # topk10_iids = tiids[topk10.indices]
        # topk5_iids = tiids[topk5.indices]
        # topk1_iids = tiids[topk1.indices]
        ...
        top20 = scores.topk(20, dim=0)
        topk10 = scores.topk(10, dim=0)
        topk5 = scores.topk(5, dim=0)
        topk1 = scores.topk(1, dim=0)

        topk20_iids = tiids[top20.indices]
        topk10_iids = iids[topk10.indices]
        topk5_iids = iids[topk5.indices]
        topk1_iids = iids[topk1.indices]

        ir_r20 = (tiids.unsqueeze(0) == topk20_iids).float().max(dim=0)[0].mean()
        ir_r10 = (tiids.unsqueeze(0) == topk10_iids).float().max(dim=0)[0].mean()
        ir_r5 = (tiids.unsqueeze(0) == topk5_iids).float().max(dim=0)[0].mean()
        ir_r1 = (tiids.unsqueeze(0) == topk1_iids).float().max(dim=0)[0].mean()
        ...
        eval_result = {
            ...
            "Recall@1": ir_r1.item(),
            "Recall@5": ir_r5.item(),
            "Recall@10": ir_r10.item(),
            "Recall@20": ir_r20.item(),
            "average_score": 100.0 * (ir_r1 + ir_r5 + ir_r10 + ir_r20).item() / 4.0,
        }
...
class VQAHandler(TaskHandler):
    ...
    def eval_batch(self, model, image, language_tokens, padding_mask, labels=None, qid=None):
        ...
        if labels is not None:
            scores = beit3_utils.VQAScore()(logits, labels) * 100.0
...
class CaptioningHandler(TaskHandler):
    def __init__(self, args) -> None:
        super().__init__()
        self.predictions = []
        self.criterion = beit3_utils.BertCaptioningLoss(
            args.label_smoothing, args.drop_worst_ratio, args.drop_worst_after
        )
...
def train_one_epoch(
    ...
    log_writer: Optional[beit3_utils.TensorboardLogger] = None,
    task=None,
    mixup_fn=None,
):
    model.train(True)
    metric_logger = beit3_utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter("lr", beit3_utils.SmoothedValue(window_size=1, fmt="{value:.6f}"))
    metric_logger.add_meter("min_lr", beit3_utils.SmoothedValue(window_size=1, fmt="{value:.6f}"))
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            grad_norm = None
            loss_scale_value = beit3_utils.get_loss_scale_for_deepspeed(model)
        else:
