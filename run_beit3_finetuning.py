...
def get_args():
    ...
    parser.add_argument(
        "--sentencepiece_model", type=str, required=True, help="Sentencepiece model path for the pretrained model."
    )
...
def main(args, ds_init):
    beit3_utils.init_distributed_mode(args)

    if ds_init is not None:
        beit3_utils.create_ds_config(args)
    ...
    seed = args.seed + beit3_utils.get_rank()
    ...
    if beit3_utils.get_rank() == 0 and args.log_dir is not None:
        os.makedirs(args.log_dir, exist_ok=True)
        log_writer = beit3_utils.TensorboardLogger(log_dir=args.log_dir)
    else:
        log_writer = None
    ...
    total_batch_size = args.batch_size * args.update_freq * beit3_utils.get_world_size()
    ...
    lr_schedule_values = beit3_utils.cosine_scheduler(
        args.lr,
        args.min_lr,
        args.epochs,
    ...
    if args.eval:
        ...
        if args.task in ["nlvr2", "flickr30k", "coco_retrieval", "imagenet"]:
            ...
            beit3_utils.dump_predictions(args, result, "vqav2_test")
            exit(0)
        elif args.task in ["coco_captioning", "nocaps"]:
            predictions, _ = evaluate(data_loader_test, model, device, task_handler)
            prediction_file = beit3_utils.dump_predictions(args, predictions, "{}_test".format(args.task))
            if beit3_utils.is_main_process() and args.task == "coco_captioning":
                captioning_result = beit3_utils.coco_caption_eval(
                    args.output_dir, prediction_file, "{}_test".format(args.task)
                )
                result_file = os.path.join(args.output_dir, f"{args.task}_result.json")
                print(json.dumps(captioning_result))
                beit3_utils.write_result_to_jsonl(captioning_result, result_file)
            exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            if args.task not in ["coco_captioning", "nocaps"]:
                ...
                prediction_file = beit3_utils.dump_predictions(args, predictions, f"{args.task}_val_e{epoch}")
                ...
                if not beit3_utils.is_main_process():
                    test_stats = beit3_utils.read_result_from_jsonl(result_file)
        ...
        if args.output_dir and beit3_utils.is_main_process():
            if log_writer is not None:
                log_writer.flush()
